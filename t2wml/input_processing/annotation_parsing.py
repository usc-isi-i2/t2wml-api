...
class DynamicallyGeneratedAnnotation:
    ...
    def _get_qualifier_yaml(self, qualifier_region, data_region):
        propertyLine, optionalsLines = self.get_optionals_and_property(qualifier_region, use_q=True)
        region=None
        ...
        if qualifier_region.is_2D:
            ...
            if alignment == False:
                ...
                region=YamlFormatter.get_qualifier_region_string(left, right, top, bottom)
        else:
    ...
    def _generate_yaml(self, data_region, subject_region, qualifier_regions):
        ...
        propertyLine, optionalsLines = self.get_optionals_and_property(data_region, use_q=False)

        if len(qualifier_regions):
            qualifierLines="qualifier:"
            for qualifier in qualifier_regions:
                qualifierLines+=self._get_qualifier_yaml(qualifier, data_region)
        else:
            qualifierLines=""
    ...
    def generate_yaml(self, sheet=None):
        ...
        for data_region in data_regions:
            for subject_region in subject_regions:
                yaml = self._generate_yaml(data_region, subject_region, qualifier_regions)
